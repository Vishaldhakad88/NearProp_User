...
function FilteredProperties() {
  ...
  // Fetch properties when specific formData fields or currentPage change
  useEffect(() => {
  ...
  const applyClientSideFilters = (props) => {
    ...
    if (formData.priceRange[0] > 0 || formData.priceRange[1] < 125000) {
      filtered = filtered.filter(p => {
        const price = p?.lowestPrice || p?.price || p?.pricePerEvent || 0;
        return price >= formData.priceRange[0] && price <= formData.priceRange[1];
      });
    }
    ...
    if (formData.availability) {
      filtered = filtered.filter(p =>
        (p?.hasAvailability || p?.isAvailable) &&
        (p?.availabilityStatus === formData.availability || p?.availability === formData.availability)
      );
    }
    ...
    filtered = filtered.sort((a, b) => {
      ...
      return formData.sortBy === 'createdAt-desc' ? dateB - dateA : dateA - dateB;
    });

    return filtered;
  };

  // Fetch properties from appropriate API
    ...
    try {
      ...
      if (formData.propertyType === 'Room') {
        ...
        const queryParams = new URLSearchParams({
          city: formData.city || '',
          minPrice: formData.priceRange[0].toString(),
          maxPrice: formData.priceRange[1].toString(),
      ...
      const response = await fetch(`${endpoint}${queryParamsString ? `?${queryParamsString}` : ''}`, {
        headers,
  ...
  // Handle search: Update URL with filter parameters
  ...
  // Remove a filter and update URL
